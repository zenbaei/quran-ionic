...
function surrondEachLineInDiv(content, pageNumber) {
    ...
    lines.forEach((str, index) => {
        if (isCenteredLine(str, pageNumber)) {
            newContent +=
                `<div style='font-size:${DEFAULT_FONT_SIZE}' class='no-justify'><nobr>${str}</nobr></div>`;
        } else {
            newContent +=
                `<div style='font-size:${getFontSize(pageNumber, index + 1)}; display: flex; justify-content: space-between; flex-direction: row-reverse;'>${str}</div>`;
        }
    });
    return newContent;
}
 ...
 * @param {*} pageNumber 
 */
function surrondFirstAndLastWordInDiv(content) {
    let lines = content.split(NEW_LINE);
    let newContent = '';
    ...
    lines.forEach((ln) => {
        ...
        if (firstWord.indexOf('<a') != -1) {
            /* fails in some cases!?
            let regex = new RegExp(REGEX_FIRST_ANCHOR);
            var result = regex.exec(ln);

            if(!result) {
                console.log(`First Match: ${firstWord} from line: ${ln}`);
                process.exit();
            }
            ...
            firstWord = '<a' + arr[1]; // 0 index is empty!!!
            //console.log(`First Match: ${firstWord}`);
        }

        if (lastWord.indexOf('</a>') != -1) {
            //let regex = new RegExp(REGEX_LAST_ANCHOR);
            //let arr = regex.exec(ln);
            let arr = ln.split('<a');
            
            if (!arr) {
                console.log(`Last Match: ${lastWord}, from line ${ln}`);
                process.exit();
            }

            lastWord = '<a' + arr[arr.length - 1];
        }

        //console.log(`First Match: ${firstWord}`);
        //console.log(`Last Match: ${lastWord}`);

        ln = ln.replace(new RegExp(`^${firstWord}`), '');
        ln = ln.replace(new RegExp(`${lastWord}$`), '');      
        ln = `<div>${firstWord}</div><div style='flex-grow:0.7'>${ln}</div><div>${lastWord}</div>`
        newContent += ln + NEW_LINE;
    });
...
const REGEX_FIRST_ANCHOR = '^(<a.*?<\/a>)';
const REGEX_LAST_ANCHOR = '(<a.*<\/a>)$';//brings all n not last!
...
module.exports = {
    ...
    surrondFirstAndLastWordInDiv: surrondFirstAndLastWordInDiv
}